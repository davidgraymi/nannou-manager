...
listen('compile-progress', (event) => {
  ...
  // No sidebar re-render: artifact count is not displayed in the sidebar,
  // and rebuilding the project list mid-hover breaks :hover state and clicks.
});
...
function setRingProgress(name, progress) {
  ...
  document
    .querySelectorAll(`.ring-fg[data-ring-name="${CSS.escape(name)}"]`)
    .forEach((fg) => { fg.style.strokeDashoffset = offset; });
}
...
function renderSidebar() {
  ...
  for (const project of projects) {
    ...
    const status = isRunning
      ...
      : isCompiling
        ? compileRingHtml(project.name, escHtml(project.name))
        : '';
