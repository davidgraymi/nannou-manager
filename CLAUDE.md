...
- `crates/cli` (`nannou-manager-cli`, binary `nou`) — thin `clap` wrapper. Each subcommand (`list`, `new`, `run`, `open`, `delete`, `copy`, `clone`, `git …`, `config …`) maps to one `core` function. Bare `nou` (no subcommand) launches the desktop UI **in-process** by calling `nannou_manager_desktop::run()` — but only when built with the `desktop` Cargo feature. That feature is off by default (so `cargo install nannou-manager-cli` stays headless and pulls no Tauri/webview deps); a headless `nou` prints an opt-in hint and exits non-zero for the bare invocation. Build the combined CLI+GUI binary with `--features desktop`; distribution packages (Homebrew cask) enable it. Integration tests in `tests/cli.rs` use `assert_cmd` against a `tempfile` projects directory.
- `crates/desktop` (`nannou-manager-desktop`) — Tauri 2 app, structured as **both a library and a binary**. `src/lib.rs` holds all the logic and exposes `pub fn run()`: it wires `core` functions as Tauri commands and adds desktop-only concerns — tracking spawned project `Child` processes in `AppState` (`running` and `compiling` maps behind `Arc<Mutex<…>>`), streaming `cargo build` artifact progress back to the UI via `Emitter`, and cancelling in-flight compiles. `src/main.rs` is a thin `fn main() { nannou_manager_desktop::run() }` (carrying the Windows `windows_subsystem` attribute) so the standalone app still builds; the `cli` crate consumes the same `run()` via its optional `desktop` feature. UI assets live in `crates/desktop/ui/`; Tauri config in `tauri.conf.json`.
