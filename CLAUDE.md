...
- `crates/cli` (`nannou-manager-cli`, binary `nannou-manager`) — thin `clap` wrapper. Each subcommand (`list`, `new`, `run`, `open`, `delete`, `copy`, `clone`, `git …`, `config …`) maps to one `core` function. Integration tests in `tests/cli.rs` use `assert_cmd` against a `tempfile` projects directory.
